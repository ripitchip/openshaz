...
class Extraction:
    ...
    @classmethod
    def extract_features(cls, path: Path) -> np.ndarray:
        """
        ...
        """
        y, sr = cls._import_audio(path)

        features = np.array([])

        features = np.hstack((features, cls._get_length(y)))
        features = np.hstack((features, cls._get_chroma_stft_mean_var(y, sr)))
        features = np.hstack((features, cls._get_rms_mean_var(y)))
        features = np.hstack((features, cls._get_spectral_centroid_mean_var(y, sr)))
        features = np.hstack((features, cls._get_spectral_bandwidth_mean_var(y, sr)))
        features = np.hstack((features, cls._get_spectral_rolloff_mean_var(y, sr)))
        features = np.hstack((features, cls._get_zero_crossing_rate_mean_var(y)))
        features = np.hstack((features, cls._get_harmonics_perceptrual_mean_var(y)))
        features = np.hstack((features, cls._get_tempo(y, sr)))
        features = np.hstack((features, cls._get_mfcc_mean_var(y, sr)))

        return features

    @classmethod
    def _import_audio(cls, path: Path) -> tuple[np.ndarray, int]:
        """
    ...
    @classmethod
    def _get_length(cls, y: np.ndarray) -> int:
        """
    ...
    def _get_chroma_stft_mean_var(
        cls, y: np.ndarray, sr: int, hop_length: int = 5000
    ) -> tuple[float, float]:
        """
    ...
    @classmethod
    def _get_rms_mean_var(cls, y: np.ndarray) -> tuple[float, float]:
        """
    ...
    @classmethod
    def _get_spectral_centroid_mean_var(cls, y: np.ndarray, sr: int) -> tuple:
        """
    ...
    @classmethod
    def _get_spectral_bandwidth_mean_var(cls, y: np.ndarray, sr: int) -> tuple:
        """
    ...
    @classmethod
    def _get_spectral_rolloff_mean_var(cls, y: np.ndarray, sr: int) -> tuple:
        """
    ...
    @classmethod
    def _get_zero_crossing_rate_mean_var(cls, y: np.ndarray) -> tuple:
        """
    ...
    @classmethod
    def _get_harmonics_perceptrual_mean_var(cls, y: np.ndarray) -> tuple:
        """
    ...
    @classmethod
    def _get_tempo(cls, y: np.ndarray, sr: int) -> float:
        """
    ...
    @classmethod
    def _get_mfcc_mean_var(cls, y: np.ndarray, sr: int) -> np.ndarray:
        """
